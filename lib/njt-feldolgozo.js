...
const HORGONY = /<span class="jhId" id="(SZ[0-9A-Z@]+)"><\/span>/g;
...
export function njtTetelek(nyers, forrasUrl) {
  ...
  for (let i = 0; i < talalatok.length; i += 1) {
    ...
    const veg = i + 1 < talalatok.length ? talalatok[i + 1].kezdet : nyers.length;
    const tartalom = szoveg(nyers.slice(kezdet, veg));
