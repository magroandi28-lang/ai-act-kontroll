...
export default async function EditSystemPage({ params }) {
  ...
  const { data: system } = await supabase
    .from("aic_ai_systems")
    .select("id,name,intended_purpose,lifecycle_stage,inventory_status,industry_code,system_type_id,aic_organisations(industry),aic_system_type_templates(type_code,name_hu),aic_ai_system_capabilities(capability_code),aic_system_facts(facts)")
    .eq("id", params.id)
  ...
  return (
    <main className="system-form-page">
      <section className="system-form-shell edit-system-shell">
        ...
        <EditSystemForm system={system} configurableCapabilities={configurableCapabilities} />
      </section>
    </main>
  );
}
